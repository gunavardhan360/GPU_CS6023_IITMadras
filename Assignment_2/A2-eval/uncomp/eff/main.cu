...
{
    //returns true for smaller vertex id's at y loc
    if (a.y != b.y)
        return a.y < b.y;

    //  if vertex id's at y are same then return true for smaller vertex id's at x loc 
    if (a.x != b.x)
        return a.x < b.x;
return true;
}

__global__ void pull_based_Adds(int *gpuOA, int *gpuCA, int *gpulocals,int *gpucurrentupdate){
  
  unsigned int id = blockIdx.x;

  // for each neighour of vertex id pull the gpucurrentupdate[i] and place it in the shared memory

  __shared__ int s;          // shared  
  ...
  s=0;
  // placing all the fetched data into the shared memory..
  for(int i=start;i<end;i++){
    s += gpucurrentupdate[ gpuCA[i] ];
  }
...
__global__ void pull_based_Mins(int *gpuOA, int *gpuCA, int *gpulocals,int *gpucurrentupdate){
  ...
  // placing all the fetched data into the shared memory..
  for(int i=start;i<end;i++){
     s = min( s, gpucurrentupdate[ gpuCA[i] ] );
  }

  // updated the local value of vertex no id
  gpulocals[id] = min(gpulocals[id],s);

}

__global__ void pull_based_Maxs(int *gpuOA, int *gpuCA, int *gpulocals,int *gpucurrentupdate){
  ...
  // placing all the fetched data into the shared memory..
  for(int i=start;i<end;i++){
    s = max( s, gpucurrentupdate[ gpuCA[i] ] );
  }

  // updated the local value of vertex no id
  gpulocals[id] = max(gpulocals[id],s);
...
int main(int argc,char **argv){
	...
	// create the CSR such that each vertexs neighborhood list is from where it can pull the updates/data..
  ...
  // open the output.txt to write the query results
    ...
    if(op == 0)	{
		gettimeofday(&t1, 0);	
		pull_based_Adds<<<n,1>>>(gpuOA,gpuCA,gpulocals,gpucurrentupdate);
		cudaDeviceSynchronize();
		gettimeofday(&t2, 0);
		}
    if(op == 1)	{
		gettimeofday(&t1, 0);
		pull_based_Mins<<<n,1>>>(gpuOA,gpuCA,gpulocals,gpucurrentupdate);
		cudaDeviceSynchronize();
		gettimeofday(&t2, 0);
		}
    if(op == 2)	{	
		gettimeofday(&t1, 0);
		pull_based_Maxs<<<n,1>>>(gpuOA,gpuCA,gpulocals,gpucurrentupdate);
		cudaDeviceSynchronize();
		gettimeofday(&t2, 0);	
		}
