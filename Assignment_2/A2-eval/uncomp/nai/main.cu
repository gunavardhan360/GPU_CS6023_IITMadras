...
{
    // returns true for smaller vertex id's at x loc
    if (a.x != b.x)
        return a.x < b.x;
 
   // if vertex id's at x are same then return true for smaller vertex id's at y loc
    if (a.y != b.y)
        return a.y < b.y;
...
__global__ void push_based_Adds(int *gpuOA, int *gpuCA, int *gpulocals,int *gpucurrentupdate){
  
  unsigned int id = blockIdx.x;

  // for each neighour of vertex id push the gpucurrentupdate[id] to its neighbors
  int valtobepushed = gpucurrentupdate[id];

  // compute the neighbors from the CSR
  int start = gpuOA[id];
  int num_neighbors = gpuOA[id+1] - gpuOA[id];
  int end = start + num_neighbors;

  //push the updates to the neighbors
  for(int i=start;i<end;i++){
    int destvertex = gpuCA[i];
    atomicAdd( gpulocals+destvertex, valtobepushed);
  }

}

__global__ void push_based_Mins(int *gpuOA, int *gpuCA, int *gpulocals,int *gpucurrentupdate){
  ...
  for(int i=start;i<end;i++){
    int destvertex = gpuCA[i];
    atomicMin( gpulocals+destvertex, valtobepushed);
  }

}

__global__ void push_based_Maxs(int *gpuOA, int *gpuCA, int *gpulocals,int *gpucurrentupdate){
  ...
  for(int i=start;i<end;i++){
    int destvertex = gpuCA[i];
    atomicMax( gpulocals+destvertex, valtobepushed);
  }
...
int main(int argc,char **argv){
	...
	// create the CSR such that each vertexs neighborhood list is where it can push the data..
  ...
  // open the output.txt to write the query results
    ...
    if(op == 0)	{
		gettimeofday(&t1, 0);	
		push_based_Adds<<<n,1>>>(gpuOA,gpuCA,gpulocals,gpucurrentupdate);
		cudaDeviceSynchronize();
		gettimeofday(&t2, 0);
		}
    if(op == 1)	{
		gettimeofday(&t1, 0);
		push_based_Mins<<<n,1>>>(gpuOA,gpuCA,gpulocals,gpucurrentupdate);
		cudaDeviceSynchronize();
		gettimeofday(&t2, 0);
		}
    if(op == 2)	{	
		gettimeofday(&t1, 0);
		push_based_Maxs<<<n,1>>>(gpuOA,gpuCA,gpulocals,gpucurrentupdate);
		cudaDeviceSynchronize();
		gettimeofday(&t2, 0);
		}
    
		double time = (1000000.0*(t2.tv_sec-t1.tv_sec) + t2.tv_usec-t1.tv_usec)/1000.0; // Time taken by kernel in milliseconds 
