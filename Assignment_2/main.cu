// Code by CH18B035 @ Gunvardhan Reddy
#include <cstdio>
#include<stdio.h>
#include<stdlib.h>
#include<cuda.h>
...
__global__ void dkernel_Adds(int *gpuOA, int *gpuCA, int *gpulocals,int *gpucurrentupdate){
	...
	unsigned tid = blockIdx.y * blockDim.x + threadIdx.x;
	unsigned start = gpuOA[id];
	unsigned end = gpuOA[id+1];
	if(tid < end && tid >= start) atomicAdd(&gpulocals[id], gpucurrentupdate[gpuCA[tid]]);
}

// complete the following kernel...
__global__ void dkernel_Mins(int *gpuOA, int *gpuCA, int *gpulocals,int *gpucurrentupdate){
	unsigned id = blockIdx.x;
	unsigned tid = threadIdx.x * blockDim.y + threadIdx.y;
	unsigned start = gpuOA[id];
	unsigned end = gpuOA[id+1];
	if(tid < end && tid >= start)
		atomicMin(&gpulocals[id], gpucurrentupdate[gpuCA[tid]]);
}
...
__global__ void dkernel_Maxs(int *gpuOA, int *gpuCA, int *gpulocals,int *gpucurrentupdate){
	...
	unsigned start = gpuOA[id];
	unsigned end = gpuOA[id+1];
	if(tid < end && tid >= start) atomicMax(&gpulocals[id], gpucurrentupdate[gpuCA[tid]]);
}
...
int main(int argc,char **argv){
  ...
  // open the output.txt to write the query results
    ...
    if(op == 0)	{
		gettimeofday(&t1, 0);	
		dkernel_Adds<<<grid,block>>>(gpuOA,gpuCA,gpulocals,gpucurrentupdate);
		cudaDeviceSynchronize();
		gettimeofday(&t2, 0);
		}
    if(op == 1)	{
		gettimeofday(&t1, 0);
		dkernel_Mins<<<grid,block>>>(gpuOA,gpuCA,gpulocals,gpucurrentupdate);
		cudaDeviceSynchronize();
		gettimeofday(&t2, 0);
		}
    if(op == 2)	{	
		gettimeofday(&t1, 0);
		dkernel_Maxs<<<grid,block>>>(gpuOA,gpuCA,gpulocals,gpucurrentupdate);
		cudaDeviceSynchronize();
		gettimeofday(&t2, 0);
		}
